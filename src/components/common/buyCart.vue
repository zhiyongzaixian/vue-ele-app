    ...
    <section v-if="!foods.specifications.length" class="cart_button">
      <transition name="showReduce">
        <span
          ...
          <!--定义减 的组件-->
          <svgImage className="add_icon" type="#icon-jianhao1"/>
        </span>
      </transition>
      ...
      <!--定义加的组件-->
      <span  @touchstart="addToCart(foods.category_id, foods.item_id, foods.specfoods[0].food_id, foods.specfoods[0].name, foods.specfoods[0].price, '', foods.specfoods[0].packing_fee, foods.specfoods[0].sku_id, foods.specfoods[0].stock, $event)">
         <svgImage className="add_icon" type="#icon-add"/>
      </span>
    </section>
      ...
      <section class="choose_icon_container">
        ...
        <transition name="showReduce">
          <span v-if="foodNum" @click="showReduceTip">
            <svgImage className="specs_reduce_icon"  type="#icon-jianhao1"/>
          </span>
        </transition>
        <transition name="fade">
          <span class="cart_num" v-if="foodNum">{{foodNum}}</span>
...
<style lang="scss" scoped>
  ...
  @include wh(.9rem, .9rem);
    ...
    background: #b4eee6;  /*zhiyong: 测试样式*/
  }
  .specs_reduce_icon {
    fill: #999;
  }
